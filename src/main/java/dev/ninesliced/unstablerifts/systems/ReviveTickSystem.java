...
public final class ReviveTickSystem extends EntityTickingSystem<EntityStore> {
    ...
    private void performRevive(@Nonnull DeadPlayerInfo deadInfo,
                               ...
                               @Nonnull GameManager gameManager,
                               @Nonnull CommandBuffer<EntityStore> commandBuffer) {
        Vector3d revivePosition = gameManager.getReviveMarkerService().getReviveMarkerPosition(deadInfo.uuid);
        ...
        gameManager.getReviveMarkerService().despawnReviveMarker(commandBuffer, deadInfo.uuid);
    ...
    private DeadPlayerInfo findNearestDeadPlayer(@Nonnull Vector3d fromPos,
        ...
        for (UUID deadUuid : deadPlayers) {
            ...
            Vector3d deathPos = gameManager.getReviveMarkerService().getReviveMarkerPosition(deadUuid);
