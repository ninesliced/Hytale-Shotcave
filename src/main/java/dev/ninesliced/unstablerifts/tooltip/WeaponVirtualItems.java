...
public final class WeaponVirtualItems {
     ...
     * item is a recognised UnstableRifts weapon (has BSON metadata with rarity
    ...
    static String buildDescription(@Nonnull WeaponDefinition def,
        ...
        sb.append(" | ").append(def.category().name());

        WeaponCategory cat = def.category();
        if (cat == WeaponCategory.SUMMONING) {
            appendStat(sb, "Mob HP", def.baseMobHealth(),
                    modBonus(modifiers, WeaponModifierType.MOB_HEALTH));
            appendStat(sb, "Mob Dmg", def.baseMobDamage(),
                    modBonus(modifiers, WeaponModifierType.MOB_DAMAGE));
            appendStat(sb, "Mob Life", def.baseMobLifetime(),
            ...
            double baseSpread = def.baseSpread();
            double basePrecision = def.basePrecision() >= 0
                    ? def.basePrecision()
