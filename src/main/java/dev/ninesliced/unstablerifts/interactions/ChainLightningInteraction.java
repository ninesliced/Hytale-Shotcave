...
public final class ChainLightningInteraction extends SimpleInstantInteraction {
    ...
    private String damageRoot = "Root_UnstableRifts_Taser_Chain_Damage";
    ...
    protected void firstRun(@Nonnull InteractionType type, @Nonnull InteractionContext context,
        ...
        if (this.aimAssist) {
            Vector3d assisted = AimAssistHelper.computeAssistedDirectionNearest(
                    commandBuffer, context, from, lookDir, effectiveMaxDistance);
            if (assisted != null) {
                lookDir = assisted;
            }
        }
    ...
    private RaycastHit traceFirstHit(@Nonnull CommandBuffer<EntityStore> commandBuffer,
        ...
        Vector3d missPosition = new Vector3d(from).addScaled(direction, effectiveMaxDistance);
        ...
        Vector3d searchCenter = new Vector3d(from).addScaled(direction, (double) effectiveMaxDistance * 0.5);
    ...
    private Vector3d getLookDirection(@Nonnull CommandBuffer<EntityStore> commandBuffer,
        ...
        float yaw = headRotation.getRotation().getYaw();
        float pitch = headRotation.getRotation().getPitch();
    ...
    private void spawnBeamSegment(@Nonnull Vector3d from,
        ...
        for (int i = 0; i <= steps; i++) {
            ...
            Vector3d point = new Vector3d(from).addScaled(delta, t);
    ...
    private record RaycastHit(@Nonnull Vector3d position, @Nullable Ref<EntityStore> target,
