...
public final class ModularGunShootInteraction extends SimpleInteraction {
    ...
    private static final HytaleLogger LOG = UnstableRiftsLog.forModule("GunShoot");
    ...
    private ShotHit traceShot(@Nonnull CommandBuffer<EntityStore> commandBuffer,
        ...
        Vector3d searchCenter = new Vector3d(from).addScaled(direction, (double) effectiveRange * 0.5);
        ...
        Vector3d miss = new Vector3d(from).addScaled(direction, effectiveRange);
    ...
    private void applyKnockback(@Nonnull CommandBuffer<EntityStore> commandBuffer,
        ...
        Vector3d direction = new Vector3d(targetTransform.getPosition()).subtract(attackerTransform.getPosition());
        direction.y = 0.0;
        if (direction.squaredLength() <= 0.000001) {
