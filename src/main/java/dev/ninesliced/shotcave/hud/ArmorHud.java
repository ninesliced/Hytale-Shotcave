...
public final class ArmorHud extends CustomUIHud {
    ...
    private String buildQuickStats() {
        ...
        if (totalSpeed > 0) {
            if (sb.length() > 0) sb.append("  ");
            sb.append("+").append((int) Math.round(totalSpeed * 100)).append("% Spd");
        }
        return sb.toString();
    }
    ...
    private void buildStatRow(@Nonnull UICommandBuilder ui, @Nonnull String name,
        ...
        if (isPercent) {
            ui.set("#ArmorStat" + name + "Base.TextSpans", Message.raw((int) Math.round(value * 100) + "%"));
        } else {
