...
/**
 * Block component that stores mob spawner configuration on a Shotcave_Mob_Spawner block.
...
public class MobSpawnerData implements Component<ChunkStore> {
    ...
    @Nonnull
    public static final BuilderCodec<MobSpawnerData> CODEC = BuilderCodec.<MobSpawnerData>builder(MobSpawnerData.class, MobSpawnerData::new)
    ...
    public static List<MobEntryData> parseEntries(@Nullable String raw) {
        ...
        for (String part : raw.split(";")) {
            ...
            if (kv.length == 2 && !kv[0].isBlank()) {
                int weight = 1;
                try { weight = Integer.parseInt(kv[1]); } catch (NumberFormatException ignored) {}
    ...
    public int parseSpawnCount() {
        ...
        try { return Integer.parseInt(spawnCount); } catch (NumberFormatException e) { return 1; }
    }
